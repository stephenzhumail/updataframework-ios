...
{
    NSMutableURLRequest* req = [[NSMutableURLRequest alloc] initWithURL:[NSURL URLWithString:@"http://0.0.0.0:8080/tmp.zip"]];
...
{
    NSMutableURLRequest* req = [[NSMutableURLRequest alloc] initWithURL:[NSURL URLWithString:@"http://0.0.0.0:8080/tmp2.zip"]];
    NSURLResponse* respone = nil;
    NSError* error = nil;
    NSData* data = [NSURLConnection sendSynchronousRequest:req returningResponse:&respone error:&error];
...
-(void)testFramework
{
    ...
    Class pacteraClass = NSClassFromString(@"Hello");
    if (!pacteraClass) {
    ...
    NSString* test = [pacteraObject performSelector:selector];
